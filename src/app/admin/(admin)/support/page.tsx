...
export default function AdminSupportPage() {
  ...
  const handleUpdateStatus = async (nextStatus: 'OPEN' | 'CLOSED') => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setDetailError(data.error || 'Failed to update status.');
        return;
      }
      await fetchTicketDetail(selectedTicket.id);
      await fetchTickets(statusFilter);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">Review and respond to user requests.</p>
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as 'OPEN' | 'CLOSED')}
          className="bg-[#1a1f2e] border border-white/10 rounded-xl px-3 py-2 text-sm text-white"
        >
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-[#1a1f2e] border border-white/10 rounded-2xl p-5 shadow-xl">
          ...
          {!loading && tickets.length === 0 && !error && (
            <div className="text-sm text-gray-400">No tickets in this view.</div>
          )}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {ticket.user?.name || 'User'} • {ticket.user?.email}
                        </p>
                      </div>
                      <span
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {selectedTicket.user?.name || 'User'} • {selectedTicket.user?.email}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  {selectedTicket.status === 'OPEN' ? (
                    ...
                    <button
                      ...
                      className="text-xs text-green-300 hover:text-green-200 border border-green-500/40 px-3 py-1 rounded-full"
                    >
                      Reopen Ticket
                    </button>
                  )}
