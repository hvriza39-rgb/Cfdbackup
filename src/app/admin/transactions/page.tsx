  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={fetchTransactions} className="flex items-center gap-2 px-4 py-2 bg-[#1a1f2e] text-white border border-white/10 rounded-xl hover:bg-white/5 transition-colors">
      ...
      <div className="flex gap-2">
        {['All', 'Deposit', 'Withdrawal'].map((tab) => (
          <button key={tab} onClick={() => setFilter(tab)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === tab ? 'bg-blue-600 text-white' : 'bg-[#1a1f2e] text-gray-400 hover:text-white'}`}>
            {tab}
          </button>
        ))}
      </div>
              ...
              {loading ? (
                ...
                filteredTransactions.map((tx: any) => (
                  <tr key={tx.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 font-mono text-white font-bold">${tx.amount.toLocaleString()}</td>
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-lg text-xs font-bold uppercase ${tx.status === 'Success' ? 'bg-green-500/10 text-green-400' : tx.status === 'Pending' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-red-500/10 text-red-400'}`}>
                        {tx.status}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      {tx.status === 'Pending' ? (
                        <div className="flex justify-center gap-2">
                          <button onClick={() => handleProcess(tx.id, 'approve')} disabled={!!processingId} className="p-2 bg-green-500/20 text-green-400 hover:bg-green-500 hover:text-white rounded-lg transition-all">
                            {processingId === tx.id ? <Loader2 size={18} className="animate-spin"/> : <CheckCircle size={18} />}
                          </button>
                          <button onClick={() => handleProcess(tx.id, 'reject')} disabled={!!processingId} className="p-2 bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white rounded-lg transition-all">
