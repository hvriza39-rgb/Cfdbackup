...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 1. Safety Check: If no token, go to login
      if (!token) {
      ...
      try {
        ...
        if (json.user) {
          setData({
            ...
            // ✅ We use the same logic that just worked in Debug Mode
  ...
  if (loading) return <div className="p-10 text-white flex justify-center">Loading Dashboard...</div>;
