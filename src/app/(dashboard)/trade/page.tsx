...
export default function TradePage() {
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      ...
      } catch (err) { setPriceError('Price unavailable'); }
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      toast.success(`${orderType} Order Filled!`, {
        duration: 5000,
        icon: orderType === 'BUY' ? '🚀' : '💰',
        style: { background: orderType === 'BUY' ? '#059669' : '#dc2626', color: '#fff', fontWeight: 'bold', padding: '16px' }
      });
      setAmount('');
      fetchBalance(); 
          ...
          <div className="flex items-center gap-4" ref={dropdownRef}>
            <div className="relative">
              <button 
                ...
                className="flex flex-col items-start hover:bg-white/5 p-1 px-2 rounded-lg transition-colors"
              >
                <h2 className="text-xl font-bold flex items-center gap-2">
                  {asset} <ChevronDown size={16} className={`transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </h2>
                <span className="text-[10px] text-blue-400 font-mono tracking-widest uppercase">{activeAsset?.type}</span>
              </button>

              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 bg-[#1a1f2e] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-100">
                  <div className="p-3 border-b border-white/5 flex items-center gap-2 bg-[#0b1220]">
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {filteredAssets.map((a) => (
                      <button 
                        ...
                        className="w-full p-3 hover:bg-white/5 flex justify-between items-center transition-colors border-b border-white/5"
                      >
                        <div className="text-left">
                          <p className="font-bold text-sm">{a.symbol}</p>
                          <p className="text-[10px] text-gray-500">{a.name}</p>
                        </div>
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${a.type === 'CRYPTO' ? 'bg-orange-500/10 text-orange-500' : 'bg-blue-500/10 text-blue-500'}`}>{a.type}</span>
                      </button>
                    ))}
                  </div>
                </div>
